...
- `log.level` controls structured JSON daemon logging and defaults to `info`.
  Routine successful requests complete at `debug` to avoid filling the
  container log while the UI polls. Requests returning HTTP 400 or higher,
  or taking at least one second, remain visible at `info`; panics are logged at
  `error`. Use `debug` for request-start diagnostics and full request
  completion records. Each completed HTTP request records a correlation ID,
  method, path, status, duration, and response size, and the same ID is
...
Recovery codes generated by current releases use the salted `v2` format. The
schema 38 migration retires and deletes older unsalted SHA-256 recovery-code
digests because their plaintext cannot be recovered or safely re-hashed. The
retirement count is recorded in the security audit; after upgrading, generate
fresh recovery codes from the Security page.
...
The schema migration from the v0.3 database is mostly additive (the current
schema is version 38); schema 38 intentionally retires unrecoverable legacy
recovery-code digests as described above. Migrations are forward-only: an older
