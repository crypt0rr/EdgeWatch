...
test('setup, login, and build a TCP/UDP job in the console', async ({ page }) => {
  ...
  await page.route('**/api/v1/**', async (route) => {
    ...
    if (path === '/auth/session' && method === 'GET') {
      ...
      await json({ username: 'admin', display_name: displayName, role: 'administrator', permissions: adminPermissions, csrf_token: csrf, totp_enabled: false, password_requirements: { minimum_length: 12 } })
      return
    }
    ...
    if (path === '/auth/login' && method === 'POST') {
      loggedIn = true
      csrf = 'test-csrf'
      await json({ username: 'admin', display_name: displayName, role: 'administrator', permissions: adminPermissions, csrf_token: csrf, totp_required: false })
      return
    }
  ...
  await expect(page.locator('.notification-list').getByText('Operations', { exact: true })).toBeVisible()
