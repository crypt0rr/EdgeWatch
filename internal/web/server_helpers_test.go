...
func TestRequiredPermissionAndMutationMatrix(t *testing.T) {
	cases := []struct {
		...
		{"/jobs/id/scans/latest-successful", http.MethodGet, "scans.read"},
		...
		{"/auth/totp/setup", http.MethodPost, auth.PermissionAccountSelf},
		{"/auth/totp/enable", http.MethodPost, auth.PermissionAccountSelf},
		{"/auth/totp", http.MethodDelete, auth.PermissionAccountSelf},
		{"/auth/sessions", http.MethodDelete, auth.PermissionAccountSelf},
