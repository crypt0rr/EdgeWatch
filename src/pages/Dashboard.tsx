...
function ActiveScanRow({ scan, cancelBusy, onCancel }: { scan: ActiveScan; cancelBusy: string; onCancel: (id: string) => void }) {
  ...
  return <div className="active-scan-row"><div className="active-scan-meta"><strong>{scan.job}</strong><span>{phase}{protocol} · {completed} of {total} probes · elapsed {elapsed}{batch}{liveness}</span><progress max={100} value={scan.progress_percent ?? 0} aria-label={`Progress for ${scan.job}`} />{scan.process_alive && scan.process_progress_percent !== undefined ? <small className="active-scan-output">Current Nmap process: {scan.process_progress_percent}%</small> : null}{scan.last_output ? <small className="active-scan-output" aria-live="polite">Nmap: {scan.last_output}</small> : null}<details className="active-scan-details"><summary>Show scan details</summary><dl><div><dt>Phase</dt><dd>{phase}{protocol}</dd></div><div><dt>Elapsed</dt><dd>{elapsed}</dd></div><div><dt>Progress</dt><dd>{completed} of {total} probes ({scan.progress_percent ?? 0}%)</dd></div>{scan.current_invocation && scan.total_batches ? <div><dt>Batch</dt><dd>{scan.current_invocation} of {scan.total_batches}</dd></div> : null}<div><dt>Process</dt><dd>{scan.process_alive ? `Nmap process active${scan.process_progress_percent !== undefined ? ` · ${scan.process_progress_percent}%` : ''}` : 'Waiting for process update'}</dd></div>{scan.last_output ? <div><dt>Latest Nmap output</dt><dd className="active-scan-detail-output" aria-live="polite">{scan.last_output}</dd></div> : null}</dl></details></div><button type="button" className="button danger" onClick={() => onCancel(scan.id)} disabled={cancelBusy === scan.id}>{cancelBusy === scan.id ? 'Cancelling…' : 'Cancel scan'}</button></div>
}
