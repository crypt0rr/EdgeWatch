...
export function Dashboard() {
  ...
  return <section className="page"><div className="page-heading"><div><p className="eyebrow">Monitoring console</p><h1>Good afternoon, {displayName}</h1><p className="muted">A calm view of your network’s expected surface. {notificationCount ? `${notificationCount} notification destination${notificationCount === 1 ? '' : 's'} configured.` : 'Notifications are not configured.'} {policy}</p></div><div className="heading-actions"><button className="button secondary" onClick={async () => { try { const value = await notificationTest(); setNotifyState({ text: `${value.sent} destination${value.sent === 1 ? '' : 's'} tested`, error: false }) } catch (err) { setNotifyState({ text: err instanceof Error ? err.message : 'Notification test failed', error: true }) } }}><Bell size={16} /> Test notifications</button><button className="button secondary" onClick={() => navigate('/jobs/new')}><Radar size={16} /> Configure job</button></div></div>{notifyState && <div className={notifyState.error ? 'form-error' : 'success-banner'} role={notifyState.error ? 'alert' : 'status'}>{notifyState.error ? <AlertTriangle size={16} /> : <CheckCircle2 size={16} />}{notifyState.text}</div>}{runError && <div className="form-error banner" role="alert"><AlertTriangle size={16} />{runError}</div>}{setup.data?.legacy_yaml_jobs?.length ? <div className="legacy-banner"><AlertTriangle size={17} /><span><strong>Legacy YAML jobs are inactive.</strong> Recreate {setup.data.legacy_yaml_jobs.join(', ')} in the console to resume scheduling.</span><button className="icon-button" onClick={() => setup.refetch()} aria-label="Refresh status">×</button></div> : null}
    ...
    {active.data?.scans.length ? <div className="panel active-scans-panel"><div className="panel-heading"><div><h2>Scans in progress</h2><p className="muted">Broad scans can take time; progress includes process liveness and Nmap timing updates.</p></div><Activity size={18} className="muted-icon" /></div><div className="active-scan-list">{active.data.scans.map(scan => <ActiveScanRow key={scan.id} scan={scan} cancelBusy={cancelBusy} onCancel={cancelActiveScan} />)}</div></div> : null}
...
function ActiveScanRow({ scan, cancelBusy, onCancel }: { scan: ActiveScan; cancelBusy: string; onCancel: (id: string) => void }) {
  ...
  const liveness = scan.process_alive ? ' · Nmap process active' : ''
  const cycle = scan.cycle_id ? `Cycle ${scan.cycle_completed_units ?? 0}/${scan.cycle_total_units ?? '?'} units · ${scan.cycle_status ?? 'running'}` : ''
  return <div className="active-scan-row"><div className="active-scan-meta"><strong>{scan.job}</strong><span>{phase}{protocol} · {completed} of {total} probes · elapsed {elapsed}{batch}{liveness}</span>{cycle && <small className="active-scan-output">{cycle}{scan.current_unit_ports ? ` · current scope ${scan.current_unit_ports} across ${scan.current_unit_addresses ?? 0} host${scan.current_unit_addresses === 1 ? '' : 's'}` : ''}</small>}<progress max={100} value={scan.progress_percent ?? 0} aria-label={`Progress for ${scan.job}`} />{scan.process_alive && scan.process_progress_percent !== undefined ? <small className="active-scan-output">Current Nmap process: {scan.process_progress_percent}%</small> : null}{scan.last_output ? <small className="active-scan-output" aria-live="polite">Nmap: {scan.last_output}</small> : null}<details className="active-scan-details"><summary>Show scan details</summary><dl><div><dt>Phase</dt><dd>{phase}{protocol}</dd></div><div><dt>Elapsed</dt><dd>{elapsed}</dd></div><div><dt>Progress</dt><dd>{completed} of {total} probes ({scan.progress_percent ?? 0}%)</dd></div>{cycle && <div><dt>Resumable cycle</dt><dd>{cycle}</dd></div>}{scan.current_unit_ports ? <div><dt>Current work unit</dt><dd>{scan.current_unit_ports} · {scan.current_unit_addresses ?? 0} host{scan.current_unit_addresses === 1 ? '' : 's'}</dd></div> : null}{scan.current_invocation && scan.total_batches ? <div><dt>Batch</dt><dd>{scan.current_invocation} of {scan.total_batches}</dd></div> : null}<div><dt>Process</dt><dd>{scan.process_alive ? `Nmap process active${scan.process_progress_percent !== undefined ? ` · ${scan.process_progress_percent}%` : ''}` : 'Waiting for process update'}</dd></div>{scan.last_output ? <div><dt>Latest Nmap output</dt><dd className="active-scan-detail-output" aria-live="polite">{scan.last_output}</dd></div> : null}</dl></details></div><button type="button" className="button danger" onClick={() => onCancel(scan.id)} disabled={cancelBusy === scan.id}>{cancelBusy === scan.id ? 'Cancelling…' : 'Cancel scan'}</button></div>
}
