...
export function Dashboard() {
  ...
  const activeMetricState = metricState(active, canOperate && !!session.data)
  const runningDetail = activeMetricState === 'error'
    ? <><span>Live count unavailable</span><MetricRetry onRetry={() => active.refetch()} /></>
    : activeMetricState === 'stale'
      ? <><span>{running ? `${running} in progress` : 'No scans running'} · stale</span><MetricRetry onRetry={() => active.refetch()} /></>
      : activeMetricState === 'loading'
        ? 'Loading live status…'
        : running ? `${running} in progress` : 'No scans running'
  ...
  return <section className="page">
    ...
    {canOperate && active.data?.scans.length ? <div className="panel active-scans-panel"><div className="panel-heading"><div><h2>Scans in progress</h2><p className="muted">Broad scans can take time; progress follows Nmap task updates when available and reports process liveness between them.</p></div><Activity size={18} className="muted-icon" /></div><div className="active-scan-list">{active.data.scans.map(scan => <ActiveScanRow key={scan.id} scan={scan} cancelBusy={cancelBusy} onCancel={cancelActiveScan} />)}</div></div> : null}
    <div className="dashboard-columns"><div className="panel"><div className="panel-heading"><div><h2>Jobs at a glance</h2><p className="muted">{canOperate ? 'Run or inspect any saved job.' : 'Inspect saved jobs and their baselines.'}</p></div><button className="text-button" onClick={() => navigate('/jobs')}>View all →</button></div>{jobs.isLoading ? <div className="skeleton-list" /> : jobs.error ? <QueryError message="Could not load jobs." onRetry={() => jobs.refetch()} /> : jobs.data?.jobs.length ? <div className="dashboard-jobs">{jobs.data.jobs.slice(0, 5).map(job => { const baseline = baselinePresentation(job.baseline); return <div className="dashboard-job" key={job.id}><div className="job-icon"><Radar size={17} /></div><div className="dashboard-job-info"><strong>{job.job.name}</strong><span>{job.job.targets.length} targets · {job.job.schedule}</span></div><span className={`pill ${baseline.tone}`}>{baseline.label}</span>{canOperate && <button aria-label={`Run ${job.job.name}`} className="icon-button" onClick={() => runConfiguredJob(job.id)} disabled={!!runningJob}>{runningJob === job.id ? <span className="spinner" /> : <Play size={15} />}</button>}</div> })}</div> : <div className="inline-empty">No jobs configured yet.</div>}</div>
      <div className="panel"><div className="panel-heading"><div><h2>Latest activity</h2><p className="muted">The most recent scan outcomes.</p></div><Clock3 size={18} className="muted-icon" /></div>{scans.isLoading ? <div className="skeleton-list" /> : scans.error ? <QueryError message="Could not load recent scans." onRetry={() => scans.refetch()} /> : scans.data?.scans.length ? <div className="activity-list">{scans.data.scans.slice(0, 6).map(scan => <LatestActivityRow key={scan.id} scan={scan} />)}</div> : <div className="inline-empty">Your first scan will appear here.</div>}</div></div>
  </section>
}
...
function LatestActivityRow({ scan }: { scan: Awaited<ReturnType<typeof listScans>>['scans'][number] }) {
  const content = <><span className={scan.status === 'success' ? 'activity-dot success' : scan.status === 'failed' ? 'activity-dot fail' : 'activity-dot'} /><div><strong>{scan.job}</strong><span>{scan.status === 'success' ? 'Completed successfully · Open scan details' : scan.error ?? scan.status}</span></div><time>{new Date(scan.finished_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</time></>
