...
function EvidencePorts({ title, description, ports, tone = 'muted' }: { title: string; description: string; ports: PortObservation[]; tone?: 'muted' | 'warning' }) {
...
function ProtocolCard({ protocol }: { protocol: ProtocolObservation }) {
  ...
  const evidenceLabel = `${protocol.protocol.toUpperCase()} positive port evidence`
...
function RdapPanel({ result, loading, error }: { result?: RdapResult; loading: boolean; error?: Error | null }) {
  ...
  return <div className="rdap-result"><div className="rdap-status"><Globe2 size={17} /><strong>{result.status === 'stale' ? 'Cached registry data (stale)' : result.status === 'cached' ? 'Cached registry data' : 'Registry data'}</strong>{result.fetched_at && <span className="muted">Fetched {formatDateTime(result.fetched_at)}</span>}</div><dl className="fact-grid">{[['Network', result.network_name], ['Handle', result.handle], ['Range', result.prefix || [result.start_address, result.end_address].filter(Boolean).join(' – ')], ['Country', result.country], ['Allocation', result.allocation_type], ['Registry', result.registry], ['Organizations', result.organizations?.join(', ')], ['Status', result.statuses?.join(', ')]].filter(([, value]) => value).map(([label, value]) => <div key={label}><dt>{label}</dt><dd>{value}</dd></div>)}</dl>{result.events?.length ? <div className="rdap-events">{result.events.map(event => <span key={`${event.action}-${event.date}`}>{event.action}{event.date ? ` · ${formatDate(event.date)}` : ''}</span>)}</div> : null}{result.source_url && <a href={result.source_url} target="_blank" rel="noreferrer" className="back-link">Open authoritative record ↗</a>}</div>
}
